...
DRAW_vertical           (short x)
{
   int         i           =    0;
   DRAW_single ('‰', x,  1);
   for (i = 2; i <= my.y_end - 2; ++i) DRAW_single ('', x,  i);
   DRAW_single ('Š', x,  4);
   DRAW_single ('ˆ', x,  my.y_end - 1);
   return 0;
}
...
DRAW_wrap               (void)
{
   ...
   if (strchr ("fs", my.d_ornament) != NULL) {
      ...
      /*> yASCII_print  (my.x_min - 2 + 13 + my.x_wide,  2, "‰", YASCII_CLEAR);       <*/
...
DRAW_boxes_OLD          (void)
{
   ...
   for (x_lvl = 0; x_lvl < MAX_LEVEL; ++x_lvl) {
      ...
      for (i = 0; i <  g_nnode; ++i) {
         ...
         /*---(make box)-----------------*/
         yASCII_print ((x_lvl * 16), 5 + (c * 5), "ƒ€€€€€€€€€€‚", YASCII_CLEAR);
         yASCII_print ((x_lvl * 16), 6 + (c * 5), "          ", YASCII_CLEAR);
         yASCII_print ((x_lvl * 16), 7 + (c * 5), "„€€€€€€€€€€…", YASCII_CLEAR);
         ystrlcpy (x_name, g_nodes [i].n_name, 11);
         l = strlen (x_name);
         l = 6 - (l / 2);
         yASCII_print ((x_lvl * 16) + l, 6 + (c * 5), x_name, YASCII_CLEAR);
         /*> yASCII_connector    (x_left + 56, x_topp + 18, 'Ö', x_left + 71, x_topp + 18, '-', "mate"      , x_left + 61, x_topp + 17);   <*/
         ++c;
      }
   }
   /*> yASCII_uconnect (12,  6, 'Ö', 16,  6);                                         <*/
   /*> yASCII_uconnect (12, 11, 'Ö', 14,  6);                                         <*/
   rc = yASCII_write (YSTR_CLIP);
   rc = yASCII_free  ();
   return 0;
}
...
DRAW_box                (char a_col, char a_row, char a_name [LEN_TITLE], char a_npred, char a_nsucc)
{
   ...
   y_save = y;
   /*---(top)----------------------------*/
   sprintf (x_line, "ƒ%*.*s‚", my.x_side, my.x_side, YSTR_HORZ);
   yASCII_print (x, y, x_line, YASCII_CLEAR);
   /*---(middle)-------------------------*/
   sprintf (x_line, "%*.*s", my.x_side, my.x_side, YSTR_EMPTY);
   for (i = 1; i <= my.y_side; ++i) {
      yASCII_print (x, y + i, x_line, YASCII_CLEAR);
   }
   y += my.y_side;
   /*---(bottom)-------------------------*/
   sprintf (x_line, "„%*.*s…", my.x_side, my.x_side, YSTR_HORZ);
   yASCII_print (x, ++y, x_line, YASCII_CLEAR);
   /*---(note)---------------------------*/
...
static char s_mapping [LEN_LABEL][LEN_TITLE] = {
   ...
   /*  ‡  */  { "‡ Š ‡ Š ‡ ‡ Š ‡ Š Š ‡ Š Š Š ‡ " },
   /*  ‰  */  { "‰ ‰ Š ‰ Š ‰ Š Š ‰ Š Š ‰ Š Š ‰ " },
   /*  ˆ  */  { "ˆ ˆ Š ˆ Š Š ˆ ˆ Š Š Š Š ˆ Š ˆ " },
...
DRAW_xconnect           (short bx, short by, short ex, short ey, char a_tall, char a_blane, char a_vlane, char a_elane)
{
   ...
   case 'Ô' : 
      ...
      DRAW_single ('…', ex - 2, by + a_blane);
      for (i = by + a_blane - 1; i >= ey + a_elane + 1; --i)  DRAW_single ('Œ', ex - a_vlane, i);
      DRAW_single ('ƒ', ex - 2, ey + a_elane);
      break;
   case 'Õ' :
      DEBUG_PROG   yLOG_note    ("descending/downward line");
      DRAW_single ('‚', bx + a_vlane, by + a_blane);
      for (i = by + a_blane + 1; i <= ey + a_elane - 1; ++i)  DRAW_single ('Œ', bx + a_vlane, i);
      DRAW_single ('„', bx + 2, ey + a_elane);
...
DRAW_hconnect_OLD       (char a_bcol, char a_brow, char a_ecol, char a_erow)
{
   ...
   /*---(direction)----------------------*/
   if      (by == ey)  { x_dir = 'Ö';  x_beg = x_end = by;  }
   else if (by <  ey)  { x_dir = 'Õ';  x_beg = by + 1;  x_end = ey - 1;  }
   else                { x_dir = 'Ô';  x_beg = ey + 1;  x_end = by - 1;  }
   /*---(always)-------------------------*/
   yASCII_print (bx    , by, "‡²", YASCII_CLEAR); 
   yASCII_print (ex - 1, ey, "²†", YASCII_CLEAR); 
   /*---(ends)---------------------------*/
   switch (x_dir) {
   case 'Ö' : 
      DEBUG_PROG   yLOG_note    ("horizontal");
   ...
   case 'Ô' : 
      ...
      DRAW_single ('…', ex - 2, by);
      for (i = by - 1; i >= ey + 1; --i)  DRAW_single ('Œ', ex - 2, i);
      DRAW_single ('ƒ', ex - 2, ey);
      break;
   case 'Õ' :
      DEBUG_PROG   yLOG_note    ("descending/downward line");
      DRAW_single ('‚', bx + 2, by);
      for (i = by + 1; i <= ey - 1; ++i)  DRAW_single ('Œ', bx + 2, i);
      DRAW_single ('„', bx + 2, ey);
